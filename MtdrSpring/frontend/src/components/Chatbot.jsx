...
function ChatBubble({ role, content, isLoading = false }) {
  return (
    <div className={`${role === 'user' ? 'col-start-1 col-end-8' : 'col-start-6 col-end-13'} p-1 sm:p-2 rounded-lg`}>
      <div className={`flex ${role === 'user' ? 'flex-row' : 'flex-row-reverse'} items-center`}>
        <div className={`flex items-center justify-center rounded-full flex-shrink-0
                        h-6 w-6 sm:h-8 sm:w-8 ${role === 'user' ? 'bg-indigo-300' : 'bg-indigo-500'}`}>
          <span className="text-xs sm:text-sm font-semibold">{role[0].toUpperCase()}</span>
        </div>
        <div className={`relative ${role === 'user' ? 'ml-2 sm:ml-3' : 'mr-2 sm:mr-3'} 
                        p-2 sm:p-3 shadow rounded-xl 
                        max-w-[70%] sm:max-w-[70%] 
                        text-xs sm:text-sm 
                        ${role === 'user' ? 'bg-white dark:bg-cyan-900 text-black dark:text-white' : 'bg-blue-500 dark:bg-sky-900 text-white'}`}>
          {isLoading ? (
...
const ChatInput = forwardRef(({ onSendMessage, disabled = false }, ref) => {
  ...
  return (
    <div className="flex items-center gap-2">
      <input
        ...
        placeholder={disabled ? "Enviando mensaje..." : "Escribe un mensaje..."}
      />
      <button
      ...
      <div className="flex-1 overflow-y-auto p-2 sm:p-4">
        {messages.length === 0 && (
          <div className="text-center text-gray-500 dark:text-gray-400 mt-4 sm:mt-8 px-4">
            <p className="mb-2 text-sm sm:text-base">👋 ¡Hola! Soy un chatbot potenciado por Gemini</p>
            <p className="text-xs sm:text-sm">Escribe un mensaje para comenzar la conversación</p>
          </div>
        )}
      ...
      <div className="p-2 sm:p-4 border-t dark:border-gray-700 bg-white dark:bg-gray-800">
        {!isGeminiAvailable && (
          <div className="mb-2 sm:mb-3 p-2 bg-yellow-100 border border-yellow-300 rounded-lg text-yellow-800 text-xs sm:text-sm">
            ⚠️ Configura tu API key de Gemini en las variables de entorno (REACT_APP_GEMINI_API_KEY)
          </div>
        )}
        <ChatInput 
